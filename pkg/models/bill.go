...
type ReceivedBill struct {
	...
	// 账单类型 1:微信,2:支付宝
	PayType int `gorm:"column:pay_type;type:tinyint(2);index:idx_received_pay_bill_id;not null" json:"pay_type"`
	// 支付宝或微信用户支付id，前端App通过hook方式可以拿到
	UserPayId string `gorm:"column:user_pay_id" json:"user_pay_id"`
	// 支付宝或微信的账单Id
	BillId string `gorm:"column:bill_id;type:varchar(191);index:idx_received_pay_bill_id;not null" json:"bill_id"`
